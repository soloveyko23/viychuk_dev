    ...
    (() => {
        ...
        function initSliders() {
            if (document.querySelector(".swiper")) {
                new swiper_core_Swiper(".main-hero__slider", {
                ...
                new swiper_core_Swiper(".new-arrivals__slider", {
                    ...
                    scrollbar: {
                        el: ".new-arrivals .swiper-scrollbar",
                        draggable: true
                    },
                    breakpoints: {
                        320: {
                ...
                new swiper_core_Swiper(".reviewed-products__slider", {
                    modules: [ Navigation, Autoplay, Scrollbar ],
                    observer: true,
                    observeParents: true,
                    slidesPerView: 4.1,
                    ...
                    speed: 800,
                    navigation: {
                        prevEl: ".reviewed-products .swiper-button-prev",
                        nextEl: ".reviewed-products .swiper-button-next"
                    },
                    scrollbar: {
                        el: ".reviewed-products .swiper-scrollbar",
                        draggable: true
                    },
                    breakpoints: {
                        320: {
                ...
                if (window.matchMedia("(min-width: 479.98px)").matches) new swiper_core_Swiper(".reviewed-products-adds__slider", {
                    ...
                    speed: 800,
                    navigation: {
                        prevEl: ".reviewed-products-adds .swiper-button-prev",
                        nextEl: ".reviewed-products-adds .swiper-button-next"
                    },
                    scrollbar: {
                        el: ".reviewed-products-adds .swiper-scrollbar",
                        draggable: true
                    },
                    breakpoints: {
                        320: {
...
***************************************************************************** */
        ...
        const toggleAuthForms = () => {
            ...
            const formReg = document.querySelector(".offcanvas-auth--registration");
            const formLogin = document.querySelector(".offcanvas-auth--login");
            if (buttonLogin && buttonReg) {
        ...
        const initSearchBlock = () => {
            const searchButtons = document.querySelectorAll(".search-block-small-button");
            const searchBlock = document.querySelector(".search-block");
            const searchWrap = document.querySelector(".search-block__wrap");
            if (!searchButtons.length || !searchBlock || !searchWrap) {
                console.warn("Search button, block, или wrap не найдены");
                return;
            }
            const handleOutsideClick = event => {
                console.log("Клик обнаружен", event.target);
                if (!searchWrap.contains(event.target) && !Array.from(searchButtons).some((btn => btn.contains(event.target)))) {
                    console.log("Клик вне блока");
                    document.documentElement.classList.remove("search-full-show");
                    document.documentElement.classList.remove("lock");
                    document.removeEventListener("click", handleOutsideClick);
                }
            };
            const toggleSearchBlock = event => {
                event.preventDefault();
                const isExpanded = document.documentElement.classList.contains("search-full-show");
                if (isExpanded) {
                    document.documentElement.classList.remove("search-full-show");
                    document.documentElement.classList.remove("lock");
                    document.removeEventListener("click", handleOutsideClick);
                } else {
                    document.documentElement.classList.add("search-full-show");
                    document.documentElement.classList.add("lock");
                    setTimeout((() => {
                        document.addEventListener("click", handleOutsideClick);
                    }), 0);
            ...
            searchButtons.forEach((btn => {
                btn.addEventListener("click", toggleSearchBlock);
            }));
        };
        const initButtonToTop = () => {
            ...
            if (buttonToTop) {
                window.addEventListener("scroll", (function() {
                    if (window.scrollY > 200) buttonToTop.style.display = "block"; else buttonToTop.style.display = "none";
                }));
            ...
            } else console.error("Button .button-to-top not found in the document.");
        };
        const presentBlockInForm = () => {
            const rowPresents = document.querySelectorAll(".row-present");
            if (rowPresents.length > 0) rowPresents.forEach((row => {
                const content = row.querySelector(".row-present__content");
                const t = row.querySelector(".row-present__button");
                const button = row.querySelector(".button-show-forms");
                const btnClose = row.querySelector(".btn-close");
                if (content && button) {
        ...
        const promoBlockInForm = () => {
            const rowPresents = document.querySelectorAll(".promo-block");
            if (rowPresents.length > 0) rowPresents.forEach((row => {
                const content = row.querySelector(".column-goods__promo-content");
                const button = row.querySelector(".button-show-promo");
                if (content && button) button.addEventListener("click", (() => {
        ...
        const initCheckboxSync = () => {
            const synchronizeCheckboxes = () => {
                ...
                checkboxes.forEach((checkbox => {
                    checkbox.addEventListener("change", (() => {
                        ...
                        checkboxes.forEach((cb => {
                            if (cb.getAttribute("data-tag") === tag && cb.getAttribute("data-group") === group) cb.checked = isChecked;
                        }));
                        updateGroupContent(group, checkboxes);
                    }));
                }));
            };
            const updateGroupContent = (group, checkboxes) => {
                const groupContainer = document.querySelector(`[data-group-content="${group}"]`);
                if (!groupContainer) return;
                const groupContent = groupContainer.querySelector(".item-filter__checked");
                if (!groupContent) return;
                const selectedNames = new Set;
                checkboxes.forEach((cb => {
                    if (cb.getAttribute("data-group") === group && cb.checked) {
                        const labelElement = cb.closest(".checkbox")?.querySelector(".checkbox__text");
                        if (labelElement) selectedNames.add(labelElement.textContent.trim());
                    }
                }));
                groupContent.textContent = Array.from(selectedNames).join(", ");
            };
        ...
        if (stickyInfo) {
            const onScroll = () => {
                if (window.scrollY >= 750) stickyInfo.classList.add("show"); else stickyInfo.classList.remove("show");
            };
